...
// The hub stores and indexes recordings in its own local time, so we use
// local (not UTC) date components here.
function toReolinkTime(date: Date) {
  return {
...
function hasFiles(result: unknown): boolean {
...
function signSession(role: string): string {
...
function verifySession(value: string): { role: string } | null {
...
// Timing-safe password check: compares HMACs so neither length nor content leaks.
...
app.post('/api/login', (req, res) => {
  ...
  if (safeCompare(password, adminPassword))       role = 'admin';
  else if (safeCompare(password, viewerPassword)) role = 'viewer';
  if (!role) {
  ...
  res.setHeader(
    'Set-Cookie',
    `${COOKIE_NAME}=${encodeURIComponent(signSession(role))}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${COOKIE_MAX_AGE_S}`
  );
  res.json({ role });
});
...
const MJPEG_INTERVAL_MS = 500; // delay between frame requests
...
app.get('/api/playback/:channel', (req, res) => {
  ...
  // If the browser cancels the fetch, tear down FFmpeg and the hub connection.
  // This handler is attached after hubReq is defined so we can reference it in scope.
